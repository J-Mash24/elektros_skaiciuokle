...
with tab_data:
    ...
    st.info(
        "PV modeliui reikia bent vieno referencinės elektrinės generacijos failo "
        "su stulpeliais 'Statistical Period' ir 'PV Yield (kWh)'. "
        "ESO vartojimo ir NPS / sąskaitos failai keliami žemiau į atskirus laukus."
    )
...
with tab_scenario:
    ...
    if not data_ready:
        if data_error_message:
            ...
            st.warning(
                "Modelis dar neparuoštas: įkelkite referencinės PV elektrinės "
                "generacijos duomenis skirtuke '1. Duomenys'."
            )
