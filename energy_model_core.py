...
# Default monthly distribution calibrated to the 2025 reference PV plant data
# previously used in this project. Values sum to 1.0 and can be overridden.
DEFAULT_MONTHLY_PV_SHARES = {
    1: 0.01445,
    2: 0.04463,
    3: 0.10099,
    4: 0.13927,
...
def create_synthetic_pv_for_load(
    load_df: pd.DataFrame,
    annual_specific_yield_kwh_kw: float = 904.5,
    monthly_shares: dict[int, float] | None = None,
    cfg: ModelConfig = ModelConfig(),
) -> pd.DataFrame:
    """Create a seasonal PV profile directly on the load timestamps.

    The profile does not require any uploaded PV file. Annual specific yield
    (kWh/kW/year) is allocated to months using a calibrated monthly
    distribution, then to days and intervals using astronomical day length and
    a smooth sine-shaped solar curve. For a complete 12-month series, the
    resulting specific PV generation is approximately equal to the requested
    annual specific yield.
    """
    if annual_specific_yield_kwh_kw <= 0:
        raise ValueError("Metinė specifinė PV generacija turi būti didesnė už 0.")

    shares = dict(monthly_shares or DEFAULT_MONTHLY_PV_SHARES)
    total_share = float(sum(shares.values()))
    if total_share <= 0:
        raise ValueError("Mėnesinių PV generacijos dalių suma turi būti didesnė už 0.")
    shares = {int(m): float(v) / total_share for m, v in shares.items()}

    load = load_df[["datetime", "load_kwh"]].copy()
    load["datetime"] = pd.to_datetime(load["datetime"], errors="coerce")
    load = load.dropna(subset=["datetime"]).sort_values("datetime").reset_index(drop=True)
    if load.empty:
        raise ValueError("Nėra datų, pagal kurias būtų galima sukurti PV profilį.")

    dt_hours = detect_timestep_hours(load)
    solar = build_solar_calendar(load["datetime"], cfg)
    result = load.merge(
        solar[[
            "datetime", "season", "is_day", "is_night",
            "sunrise_hour", "sunset_hour", "day_length_h", "night_length_h",
    ...
    # Monthly specific production is distributed uniformly by day. The
    # intra-day shape follows sunrise/sunset and is normalized so each day's
    # energy is preserved.
    result["daily_specific_target"] = result.apply(
        lambda r: annual_specific_yield_kwh_kw
        * shares.get(int(r["month"]), 0.0)
        / max(int(r["days_in_month"]), 1),
        axis=1,
    )

    local_hour_mid = (
        result["datetime"].dt.hour
        + result["datetime"].dt.minute / 60.0
        + result["datetime"].dt.second / 3600.0
        + dt_hours / 2.0
    )
    daylight = (result["sunset_hour"] - result["sunrise_hour"]).clip(lower=1e-9)
    phase = (local_hour_mid - result["sunrise_hour"]) / daylight * math.pi
    raw = np.where(
        (local_hour_mid >= result["sunrise_hour"])
        & (local_hour_mid < result["sunset_hour"]),
        np.sin(phase.clip(lower=0.0, upper=math.pi)),
        0.0,
    )
    result["_solar_raw"] = np.maximum(raw, 0.0)
    daily_raw_sum = result.groupby("date")["_solar_raw"].transform("sum")

    # Defensive fallback for days with no daylight interval captured.
    # This is not expected for Lithuanian hourly/15-min data, but prevents a
    # zero-division if the input contains only night-time observations.
    fallback = daily_raw_sum <= 0
    if fallback.any():
        counts = result.groupby("date")["datetime"].transform("count")
        result.loc[fallback, "_solar_raw"] = 1.0 / counts[fallback]
        daily_raw_sum = result.groupby("date")["_solar_raw"].transform("sum")

    result["pv_per_kw"] = (
        result["daily_specific_target"]
        * result["_solar_raw"]
        / daily_raw_sum.replace(0, np.nan)
    ).fillna(0.0)

    return result[[
        "datetime", "load_kwh", "pv_per_kw", "season", "is_day", "is_night",
        "sunrise_hour", "sunset_hour", "day_length_h", "night_length_h",
